...
export class PromptBuilder {
  ...
  _updateTokenLimits(provider, model) {
    ...
    const split = this.tokenLimits.calculateOptimalSplit(provider, model, this.config.systemPromptTokens);
  ...
  _getOptimizationInfo(originalData, optimizedData) {
    ...
    return {
      ...
      reductionRatio: originalSize > 0 ? (originalSize - optimizedSize) / originalSize : 0,
  ...
  /**
   * Selects appropriate prompt template based on available data
   * @private
   */
...
- Suggest ways to gather more comprehensive data for future retrospectives`,
  ...
  _buildSystemPrompt(dateRange, teamSize, repositories, channels, model) {
    ...
    const contextInfo = `
Context Information:
- Analysis Period: ${dateRange?.start && dateRange?.end ? `${dateRange.start} to ${dateRange.end}` : 'Not specified'}
...
3. **Specific Evidence**: Every insight must be supported by concrete examples from the data. Quote commit messages, reference specific issues, mention actual Slack conversations.

4. **Quantitative Context**: Include numbers, timeframes, and measurable patterns. "3 critical bugs resolved within 2 hours" is better than "quick bug fixes."

5. **Root Cause Analysis**: Don't just identify what happened - analyze why it happened and what underlying factors contributed to successes or challenges.

6. **Actionable Depth**: Recommendations should be specific enough that the team can implement them immediately. Include who, what, when, and how.

7. **Team-Level Focus**: Analyze team patterns and collective behaviors rather than individual performance. Look for systemic strengths and improvement opportunities.`;
...
Please analyze this data according to the instructions and provide comprehensive insights in the specified JSON format.`;
  ...
  optimizeDataForTokens(teamData, context) {
    ...
    if (estimatedTokens <= this.maxDataTokens) {
      console.log(`Data fits comfortably: ${estimatedTokens.toLocaleString()} tokens ≤ ${this.maxDataTokens.toLocaleString()} limit`);
      return teamData;
    }
    
    console.log(`Optimizing data for ${this.currentLimits.provider}/${this.currentLimits.model}: ${estimatedTokens.toLocaleString()} tokens > ${this.maxDataTokens.toLocaleString()} limit`);
    ...
    if (optimized.github) {
      optimized.github = this._optimizeGitHubData(optimized.github, reductionRatio);
    }
    
    if (optimized.linear) {
      optimized.linear = this._optimizeLinearData(optimized.linear, reductionRatio);
    }
    
    if (optimized.slack) {
      optimized.slack = this._optimizeSlackData(optimized.slack, reductionRatio);
    }
  ...
  _optimizeGitHubData(githubData, reductionRatio) {
    ...
    // Prioritize recent commits and important PRs
    if (optimized.commits) {
      const targetLength = Math.floor(optimized.commits.length * reductionRatio);
      optimized.commits = optimized.commits
        .sort((a, b) => new Date(b.date) - new Date(a.date))
        .slice(0, Math.max(5, targetLength));
    }
    
    if (optimized.pullRequests) {
      const targetLength = Math.floor(optimized.pullRequests.length * reductionRatio);
  ...
  _optimizeSlackData(slackData, reductionRatio) {
    ...
    if (optimized.messages) {
      const targetLength = Math.floor(optimized.messages.length * reductionRatio);
      optimized.messages = optimized.messages
        .sort((a, b) => new Date(b.ts) - new Date(a.ts))
        .slice(0, Math.max(10, targetLength))
        .map(message => ({
          ...message,
